...
#### Open the dataset ####
def prepare_dataset(dataset, train_window, hidden_size, BATCH_SIZE, model_type):
    ...
    return val_loader, test_loader, w_size, z_size, n_channels, val, test

def set_model(model_type, n_channels, train_window, w_size, z_size, checkpoint_dir):
...
def test_model(model, val_loader, test_loader, model_type, threshold_method, percentile, val, test, synth_gen, contamination, period, anom_amplitude_factor):
...
if __name__ == "__main__":
    ...
    synth_gen = args.synthetic_generation
    ...
    val_loader, test_loader, w_size, z_size, n_channels, val, test = prepare_dataset(dataset, train_window, hidden_size, BATCH_SIZE, model_type)
    model = set_model(model_type, n_channels, train_window, w_size, z_size, checkpoint_dir)
    test_model(model, val_loader, test_loader, model_type, threshold_method, percentile, val, test, synth_gen, contamination, period, anom_amplitude_factor)